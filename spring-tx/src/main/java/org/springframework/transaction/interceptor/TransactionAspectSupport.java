...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// 2、从TransactionAttributeSource获得当前执行方法对应的事务定义TransactionAttribute
		final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null);
		// 3、获得PlatformTransactionManager
		final PlatformTransactionManager tm = determineTransactionManager(txAttr);
		final String joinpointIdentification = methodIdentification(method, targetClass, txAttr);
		
		// 4、如果没有对应的事务定义，或
		// 事务管理器不是CallbackPreferringPlatformTransactionManager类型的
		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
			// Standard transaction demarcation with getTransaction and commit/rollback calls.
			// 5、应用标准的事务管理过程
			// 5.1、getTransaction TransactionInfo包装参数便于取
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		if (txAttr != null) {
			if (tm != null) {
				// 在这里调用tm.getTransaction(TransactionDefinition )
